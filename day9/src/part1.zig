...
pub fn getResult(input: []const u8) !i64 {
    var buf: [20]u8 = undefined;
    var iter = DiskMapIterator.init(input, &buf);
    var riter = DiskMapReverseIterator.init(input, &buf);
    var i: i64 = 0;
    var checksum: i64 = 0;
    while (try iter.next()) |entry| {
        ...
        if (entry == '.') {
            _ = try riter.next();
            const n = sliceToInt(riter.string_buffer);
            checksum += i * (n);
            // printLn("{c}", .{new.?});
...
test getResult {
    ...
    const answer = try getResult(test_input);
